...
function AddPlacePopup(props) {
    ...
    function handleSubmit(e) {
        ...
        props.onAddPlace(name,link,form);
    }
    ...
    return(
        <PopupWithForm title='Новое место' name='addcard' buttonvalue={!props.onLoading ? 'Создать' : 'Создать...'} isOpen = {props.isOpen} onClose = {props.onClose} onSubmit={handleSubmit}>
                <fieldset className="popup__inputarea">
                    <input id="area" type="text" name="area" className={!nameValid ? `popup__text popup__text_type_error` : `popup__text`} placeholder="Название" minLength="2" maxLength="30" onChange = {handleNameChange} required />
                    <span id="area-error" className="popup__text-error">{!nameValid ? `${nameErrorMessage}` : ``}</span>
                </fieldset>
                <fieldset className="popup__inputarea">
                    <input id="imagelink" type="url" name="imagelink" className={!linkValid ? `popup__text popup__text_type_error` : `popup__text`} placeholder="Ссылка на картинку" onChange = {handleLinkChange} required />
